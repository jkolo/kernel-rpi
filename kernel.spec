...
%endif
    ...
    # "needs unknown symbol" warnings are EXPECTED when filter-modules.sh splits
    # modules across main vs -modules-extra packages (e.g. mlx5-vfio-pci in main,
    # mlx5_core in extra; cifs.ko in main, ib_core.ko in extra). The cross-
    # package symbol resolution happens at runtime via depmod -a once both
    # packages are installed.
    #
    # Filter those out and only fail on other (real) depmod errors.
    depmod -b . -aeF ./System.map $KernelVer &> depmod.out || true
    grep -v 'needs unknown symbol' depmod.out > depmod.real || true
    if [ -s depmod.real ]; then
        echo "Depmod failure (real errors, not cross-package symbol gaps):"
        cat depmod.real
        rm -f depmod.out depmod.real
        exit 1
    else
        if [ -s depmod.out ]; then
            echo "depmod cross-package symbol warnings (non-fatal):"
            head -20 depmod.out
            echo "[+$(($(wc -l < depmod.out) - 20)) more lines]"
        fi
        rm -f depmod.out depmod.real
    fi
...
- Sync RPi patch to git revision: 42c8bbfa4cdd08eb5fe605e56ad13777559e9f24
...
* Fri Dec 12 2025 Damian Wrobel <dwrobel@ertelnet.rybnik.pl> - 6.12.61-1.rpi
- Update to stable kernel patch v6.12.61
- Sync RPi patch to git revision: 21b410140c47ffab5668399f6f143c7d7b935c8b
...
* Wed Aug 20 2025 Damian Wrobel <dwrobel@ertelnet.rybnik.pl> - 6.12.42-1.rpi
- Update to stable kernel patch v6.12.42
- Sync RPi patch to git revision: 2fa00bc2d0ecd77c12620f13dbc38aa27a42cf87
